...
conclusiones de Luis de las estadisticas
...
La moda tambien nos da una idea aproximada de los resultados vistos arriba , al haberse repetido muchas veces el 80 como Ataque de varios pokemon , eso ha influido de forma relativa en los resultados 
...
### 1.4 Distribución de las alturas
...
### 1.5 Frecuencia de los tipos elementales
...
### 1.6 Número de Pokémon por Generación
...
## 2.2 Comparativas entre estadísticas en base a la Velocidad

En esta sección del análisis, nuestro objetivo es comparar las estadísticas de ataque físico y ataque especial en función de la velocidad de los Pokémon, para luego hacer lo mismo con las estadísticas de defensa física y defensa especial. La velocidad es una característica clave que influye en el orden de los turnos durante una batalla, y es interesante explorar cómo se relaciona con las capacidades ofensivas y defensivas de los Pokémon.

Primero, analizaremos si existe una correlación entre la velocidad y las dos formas de ataque, para identificar si los Pokémon más rápidos tienden a tener un mayor ataque físico, ataque especial o si mantienen un equilibrio entre ambos. Después, realizaremos un análisis similar para la defensa física y la defensa especial, observando si los Pokémon rápidos muestran algún patrón en su capacidad de resistencia frente a ataques físicos o especiales.

Finalmente, compararemos las rectas de regresión obtenidas en estos análisis, buscando identificar patrones que nos permitan sacar conclusiones sobre la relación entre estas estadísticas clave. Esto nos permitirá entender si hay tipos de Pokémon que siguen estrategias específicas en términos de velocidad y desempeño ofensivo/defensivo, o si estos factores tienden a comportarse de manera más independiente.


### 2.2.1 La Velocidad en base al Ataque
...
Ahora pondremos los coeficientes de correlación y covarianza : 
...
Los tipos Hada (0.500), Psíquico (0.503), y Dragón (0.460) muestran una relación positiva entre velocidad y ataque, indicando que los Pokémon más rápidos de estos tipos tienden a tener un mayor ataque.

Con respecto a las covarianzas , el tipo Hada (477) y Psíquico (455) presentan las mayores covarianzas, lo que refuerza la relación entre velocidad y ataque en estos tipos.
...
### 2.2.2 La Velocidad en base a la Defensa

Ahora compararemos la Defensa y la Defensa Especial
...
Los tipos con mejor correlación, aunque más débiles, son Hada (0.392), Normal (0.343), y Lucha (0.243).

Con respecto a las covarianzas , Hada (355) y Normal (205) muestran las mayores covarianzas, pero la relación es más débil en comparación con el ataque.

Un caso curioso es que el tipo Roca presenta una correlación negativa notable (-0.377) y la mayor covarianza negativa (-393), lo que sugiere que en este tipo, los Pokémon más rápidos tienden a tener una defensa más baja, lo cual es contrario a lo esperado en términos de relación velocidad-defensa.
...
Los tipos Dragón (0.406), Hada (0.398), y Fuego (0.404) destacan por una correlación positiva entre velocidad y defensa especial.

Con respecto a las covarianzas , Hada (411) y Dragón (303) presentan las mayores covarianzas.

Un caso curioso es que el tipo Acero presenta una correlación ligeramente negativa (-0.026) con la velocidad y la defensa especial, y una covarianza negativa (-19.2), lo cual indica que los Pokémon de tipo acero más rápidos no tienen necesariamente mejores defensas especiales.


### 2.2.3 Conclusión

Tras hacer las correspondientes comparaciones , podemos observar que algunas conclusiones interesantes sobre la velocidad acorde a algunos tipos : 


Comparando todas las gráficas , podemos concluir que la mayoría de Pokémon invierten mas en Ataque en funcion de su Velocidad, debido a que es mas preferible , y tiene mas lógica que un Pokémon se mueva antes y pueda atacar con mayor cantidad. No es muy viable en general invertir en casos donde un pokemon rápido con buenas defensas , pero malos puntos de ataques. 

Los atacantes especiales, en especial los de tipo Eléctrico poseen muy buena Velocidad. Lo cual nos da a suponer que es mas viable utilizar Pokémon rápidos y con buen Ataque Especial , porque si nos fijamos en las gráficas de Defensa Especial , la media se encuentra muy por debajo en comparación a la Defensa física
...
Analizando las gráficas del tipo Eléctrico , podemos observar que tienen mucho Ataque y Ataque Especial. Eso nos da a entender que este es uno de los tipos mas ofensivos a nivel de combate, y pueden hacer un muy buen daño neutro a muchos rivales con una velocidad aceptable. 


Algunos tipos pueden tener una afinidad por sus estadísticas hacia el lado fisico mas que al especial , y viceversa. Es el caso del tipo Roca , gracias a la Velocidad podemos llegar a la conclusión de por su naturaleza mas robusta , los pokemon de tipo Roca son mas físicos que especiales. La defensa Especial no destaca por ser muy buena en estos(exclyuendo el extremo caso de Shuckle). 

También nos da un dato crucial en los tipo Roca , por la parte física : A mayor velocidad , mayor Ataque , y menor capacidad de defensa , mientras que a menor velocidad , mayor capacidad de defender, pero puede ocasionar que el pokémon no pege con el daño que el usuario desea. 
...
Otro dato interesante a resaltar es que haya pokemon con mejores estadistica que la velocidad : el caso de los pokémon tipo Acero , hay buenas estadísticas a resaltar en las diferentes áreas , pero no destacan en velocidad. De hecho , no hay ningun pokémon de tipo Acero que supere los 110 puntos de velocidad. Con lo cual , es muy probable que la mayoría de pokemon que invierten en Velocidad se muevan antes que el tipo Acero. Pero a su vez , este tipo tiene versatilidad para ser utilizado como defensivo u ofensivo dependiendod el enfoque de sus estadisticas 
...
Con respecto a los coeficientes de correlaciones y  covarianzas, los tipos Hada, Psíquico, y Dragón muestran una relación positiva significativa entre velocidad y sus estadísticas ofensivas y defensivas, destacando como los tipos con mayor consistencia en correlación y covarianza. 

Sin embargo, tipos como Roca y Acero tienen patrones inversos que pueden parecer contraproducentes, especialmente en términos de defensa, donde la velocidad parece asociarse con menores estadísticas defensivas en estos casos.Ahi es donde se ve las correlaciones inversamente proporcionales y es lo interesante de comparar.
